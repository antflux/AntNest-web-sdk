/************
* Antnest Web SDK
* https://github.com/jonsonlion/AntNest-web-sdk
************/

/**
 * Antnest object to manage the internal queue and send requests to Antnest server. 
 * @name Antnest
 * @global
 * @namespace Antnest
 * @example <caption>SDK 安装</caption>
 ...
 *  cly.src = "https://yourdomain.com/countly.min.js";
...
(function (root, factory) {
    if(typeof define === "function" && define.amd) {
      define([], function(){ return factory(root.Antnest);});
    } else if(typeof module === "object" && module.exports) {
      module.exports = factory(root.Antnest);
    } else {
      root.Antnest = factory(root.Antnest);
    }
}(typeof window !== "undefined" ? window : this, function (Antnest) {
    "use strict";
    /** @lends Antnest */
    Antnest = Antnest || {};
    ...
    var inited = false,//是否初始
        sessionStarted = false,//session 状态
        apiPath = "/i",//api 路径
        beatInterval = 500,
        queueSize = 1000,
        requestQueue = [],
     ...
     */
    Antnest.features = ["sessions", "events", "views", "scrolls", "clicks", "forms", "crashes", "attribution", "users", "star-rating", "location"];

    //create object to store consents
    var consents = {};
    for (var i = 0; i < Antnest.features.length; i++) {
        consents[Antnest.features[i]] = {};
    }

    /**
     * Initialize Antnest object
     * @param {Object} conf - Antnest initialization {@link Init} object with configuration options
     * @param {string} conf.app_key - app key for your app created in Antnest
     * @param {string} conf.device_id - to identify a visitor, will be auto generated if not provided
     * @param {string} conf.url - your Antnest server url, you can use your server URL or IP here
    ...
    Antnest.init = function (ob) {
        if (!inited) {
            ...
            Antnest.ignore_prefetch = getConfig("ignore_prefetch", ob, true);
            Antnest.debug = getConfig("debug", ob, false);
            Antnest.app_key = getConfig("app_key", ob, null);
            Antnest.test_mode = getConfig("test_mode", ob, false);
            Antnest.device_id = getConfig("device_id", ob, getId());
            Antnest.url = stripTrailingSlash(getConfig("url", ob, ""));
            Antnest.app_version = getConfig("app_version", ob, "0.0");
            Antnest.country_code = getConfig("country_code", ob, null);
            Antnest.city = getConfig("city", ob, null);
            Antnest.ip_address = getConfig("ip_address", ob, null);
            Antnest.ignore_bots = getConfig("ignore_bots", ob, true);
            Antnest.force_post = getConfig("force_post", ob, false);
            Antnest.q = Antnest.q || [];
            Antnest.onload = Antnest.onload || [];
            Antnest.ignore_visitor = getConfig("ignore_visitor", ob, false);
            Antnest.require_consent = getConfig("require_consent", ob, false);
            if (ob.ignore_referrers && ob.ignore_referrers.constructor === Array) {
                ignoreReferrers = ob.ignore_referrers;
            }
            else if (Antnest.ignore_referrers && Antnest.ignore_referrers.constructor === Array) {
                ignoreReferrers = Antnest.ignore_referrers;
            }

            if (Antnest.url === "") {
                log("Please provide server URL");
                Antnest.ignore_visitor = true;
            }
            if (store("cly_ignore")) {
                //opted out user
                Antnest.ignore_visitor = true;
            ...
            if (window.name && window.name.indexOf("cly:") === 0) {
                try{
                    Antnest.passed_data = JSON.parse(window.name.replace("cly:", ""));
                } catch(ex){log("Could not parse name", window.name);}
            }
            else if (location.hash && location.hash.indexOf("#cly:") === 0) {
                try{
                    Antnest.passed_data = JSON.parse(location.hash.replace("#cly:", ""));
                } catch(ex){log("Could not parse hash", location.hash);}
            }

            if (Antnest.passed_data) {
                if (Antnest.passed_data.token && Antnest.passed_data.purpose) {
                    if (Antnest.passed_data.token !== store("cly_old_token")) {
                        setToken(Antnest.passed_data.token);
                        store("cly_old_token", Antnest.passed_data.token);
                    }
                    if (Antnest.passed_data.purpose === "heatmap") {
                        Antnest.ignore_visitor = true;
                        showLoader();
                        loadJS(Antnest.url + "/views/heatmap.js", hideLoader);
                    }
                }
            }

            if (!Antnest.ignore_visitor) {
                log("Antnest initialized");
                //let code waiting for us to load, know that we have loaded
                if (Antnest.onload.constructor !== Array) {
                    Antnest.onload = [];
                }

                if (Antnest.q.constructor !== Array) {
                    Antnest.q = [];
                }
                
                heartBeat();
                store("cly_id", Antnest.device_id);
                var i = 0;
                Antnest.utm = Antnest.utm || {"source":true, "medium":true, "campaign":true, "term":true, "content":true};
                ...
                if (location.search) {
                    ...
                    for (i = 0; i < parts.length; i++) {
                        ...
                        else if((nv[0]+"").indexOf("utm_") === 0 && Antnest.utm[nv[0].replace("utm_", "")]){
                            utms[nv[0].replace("utm_", "")] = nv[1];
                            hasUTM = true;
                        }
                    }
                    if(hasUTM){
                        for(var tag in Antnest.utm){
                            if(utms[tag]){
                                Antnest.userData.set("utm_"+tag, utms[tag]);
                            }
                            else{
                                Antnest.userData.unset("utm_"+tag);
                            }
                        }
                        Antnest.userData.save();
                    }
                }
    ...
    * Antnest.group_features({all:["sessions","events","views","scrolls","clicks","forms","crashes","attribution","users"]});
    * //After this call Antnest.add_consent("all") to allow all features
    @example <caption>Grouping features</caption>
    * Antnest.group_features({
    *    activity:["sessions","events","views"],
    *    interaction:["scrolls","clicks","forms"]
    * });
    * //After this call Antnest.add_consent("activity") to allow "sessions","events","views"
    * //or call Antnest.add_consent("interaction") to allow "scrolls","clicks","forms"
    * //or call Antnest.add_consent("crashes") to allow some separate feature
    */
   Antnest.group_features = function (features) {
        if (features) {
            var ob;
    ...
    * @param {string} feature - name of the feature, possible values, "sessions","events","views","scrolls","clicks","forms","crashes","attribution","users" or customly provided through {@link Antnest.group_features}
    */
   Antnest.check_consent = function (feature) {
        if (!Antnest.require_consent) {
    ...
    */
   Antnest.check_any_consent = function () {
        if (!Antnest.require_consent) {
    ...
    * @param {string|array} feature - name of the feature, possible values, "sessions","events","views","scrolls","clicks","forms","crashes","attribution","users", etc or customly provided through {@link Antnest.group_features}
    */
   Antnest.add_consent = function (feature) {
        log("Adding consent for " + feature);
        if (feature.constructor === Array) {
            for (var i = 0; i < feature.length; i++) {
                Antnest.add_consent(feature[i]);
            }
        }
            ...
            if (consents[feature].features) {
                ...
                Antnest.add_consent(consents[feature].features);
            }
            else {
                ...
                if (consents[feature].optin !== true) {
                    ...
                    setTimeout(function () {
                        if (feature === "sessions" && lastParams.begin_session) {
                            Antnest.begin_session.apply(Antnest, lastParams.begin_session);
                            lastParams.begin_session = null;
                        }
                        else if (feature === "views" && lastParams.track_pageview) {
                            lastView = null;
                            Antnest.track_pageview.apply(Antnest, lastParams.track_pageview);
                            lastParams.track_pageview = null;
                        }
                        if (lastParams.change_id) {
                            Antnest.change_id.apply(Antnest, lastParams.change_id);
    ...
    * @param {string|array} feature - name of the feature, possible values, "sessions","events","views","scrolls","clicks","forms","crashes","attribution","users", etc or customly provided through {@link Antnest.group_features}
    */
   Antnest.remove_consent = function (feature) {
        log("Removing consent for " + feature);
        if (feature.constructor === Array) {
            for (var i = 0; i < feature.length; i++) {
                Antnest.remove_consent(feature[i]);
            }
        }
        else if (consents[feature]) {
            if (consents[feature].features) {
                //this is added group, let's iterate through sub features
                Antnest.remove_consent(consents[feature].features);
            }
            else {
                //this is core feature
   ...
   Antnest.begin_session = function (noHeartBeat, force) {
        if (Antnest.check_consent("sessions")) {
            if (!sessionStarted) {
    ...
    */
   Antnest.session_duration = function (sec) {
        if (Antnest.check_consent("sessions")) {
            if (sessionStarted) {
    ...
    */
    Antnest.end_session = function (sec, force) {
        if (Antnest.check_consent("sessions")) {
            if (sessionStarted) {
    ...
    **/
    Antnest.change_id = function (newId, merge) {
        if (Antnest.device_id !== newId) {
            if (!merge) {
                //empty event queue
                ...
                //end current session
                Antnest.end_session(null, true);
                //clear timed events
                timedEvents = {};
            }
            var oldId = Antnest.device_id;
            Antnest.device_id = newId;
            store("cly_id", Antnest.device_id);
            log("Changing id");
            if (merge) {
                if (Antnest.check_any_consent()) {
                ...
                Antnest.begin_session(!autoExtend, true);
            }
        }
    };
    ...
    /**
    * Report custom event
    * @param {Object} event - Antnest {@link Event} object
    ...
    **/
    Antnest.add_event = function (event) {
        if (Antnest.check_consent("events")) {
    ...
    **/
    Antnest.start_event = function (key) {
        if (timedEvents[key]) {
    ...
    /**
    * End timed event
    * @param {string|object} event - event key if string or Antnest event same as passed to {@link Antnest.add_event}
    **/
    Antnest.end_event = function (event) {
        ...
        Antnest.add_event(event);
        delete timedEvents[event.key];
    };

    /**
    * Provide information about user
    * @param {Object} user - Antnest {@link UserDetails} object
    ...
    **/
    Antnest.user_details = function (user) {
        if (Antnest.check_consent("users")) {
    ...
    **/
    Antnest.report_conversion = function (campaign_id, campaign_user_id) {
        if (Antnest.check_consent("attribution")) {
    ...
    * @namespace Antnest.userData
    * @name Antnest.userData
    * @example
    * //set custom key value property
    * Antnest.userData.set("twitter", "ar2rsawseen");
    * //create or increase specific number property
    * Antnest.userData.increment("login_count");
    * //add new value to array property if it is not already there
    * Antnest.userData.push_unique("selected_category", "IT");
    ...
    **/
    Antnest.track_errors = function (segments) {
    ...
    * @param {string=} segments - additional key value pairs you want to provide with error report, like versions of libraries used, etc.
    **/
    Antnest.log_error = function (err, segments) {
    ...
    **/
    Antnest.add_log = function (record) {
        if (Antnest.check_consent("crashes")) {
    ...
    **/
    Antnest.track_sessions = function () {
        //start session
        Antnest.begin_session();
        Antnest.start_time();
    ...
    **/
    Antnest.track_pageview = function (page, ignoreList) {
        reportViewDuration();
        ...
        //track pageview
        if (Antnest.check_consent("views")) {
            add_cly_events({
                "key": "[CLY]_view",
    ...
    * @param {array=} ignoreList - optional array of strings or regexes to test for the url/view name to ignore and not report
    **/
    Antnest.track_view = function (page, ignoreList) {
        Antnest.track_pageview(page, ignoreList);
    };
    ...
    **/
    Antnest.track_clicks = function (parent) {
        ...
        function processClick(event) {
            if (shouldProcess) {
                ...
                if (typeof event.pageX !== "undefined" && typeof event.pageY !== "undefined") {
                    ...
                    //record click event
                    if (Antnest.check_consent("clicks")) {
                        add_cly_events({
                            "key": "[CLY]_action",
                            "segmentation": {
                                "type": "click",
    ...
    * @param {Object=} parent - DOM object which children to track, by default it is document body
    **/
    Antnest.track_scrolls = function (parent) {
        ...
        function processScrollView(e) {
            if (shouldProcess) {
                ...
                if (Antnest.check_consent("scrolls")) {
                    add_cly_events({
                        "key": "[CLY]_action",
                        "segmentation": {
                            "type": "scroll",
    ...
    * @param {Object=} parent - DOM object which children to track, by default it is document body
    **/
    Antnest.track_links = function (parent) {
        parent = parent || document;
        function processClick(event) {
    ...
    * @param {Object=} parent - DOM object which children to track, by default it is document body
    **/
    Antnest.track_forms = function (parent) {
        parent = parent || document;
        ...
        function processForm(event) {
            ...
            //record submit event
            if (Antnest.check_consent("forms")) {
                add_cly_events({
                    "key": "formSubmit",
    ...
    /**
    * Collect possible user data from submitted forms. Add cly_user_ignore class to ignore inputs in forms or cly_user_{key} to collect data from this input as specified key, as cly_user_username to save collected value from this input as username property. If not class is provided, Antnest SDK will try to determine type of information automatically.
    * @param {Object=} parent - DOM object which children to track, by default it is document body
    ...
    **/
    Antnest.collect_from_forms = function (parent, useCustom) {
        parent = parent || document;
        function processForm(event) {
            var form = get_event_target(event);
            var userdata = {};
            ...
            if (hasUserInfo) {
                ...
                if (useCustom) {
                    Antnest.user_details({ custom: userdata });
                }
                else {
                    Antnest.user_details(userdata);
                }
            }
        }
    ...
    **/
    Antnest.collect_from_facebook = function (custom) {
        /* globals FB */
        if (FB && FB.api) {
    ...
    **/
    Antnest.opt_in = function () {
        store("cly_ignore", false);
        Antnest.ignore_visitor = false;
        checkIgnore();
        if (!Antnest.ignore_visitor && !hasPulse) {
            heartBeat();
        }
    };
    ...
    */
    Antnest.show_feedback_popup = function(id) {
        // define xhr object
        sendXmlHttpRequest(Antnest.url+ '/o/feedback/widget', {widget_id:id}, function(err, params, responseText){
            if(err){
    ...
    **/
    Antnest.enable_feedback = function(params){
        // inject feedback styles
        loadCSS(Antnest.url + '/star-rating/stylesheets/antnest-feedback-web.css');
        ...
        if (enableWidgets.length > 0) {
            ...
            sendXmlHttpRequest(Antnest.url + '/o/feedback/multiple-widgets-by-id', {widgets:JSON.stringify(enableWidgets)}, function(err, params, responseText){
                if(err){
    ...
    **/
    function checkIgnore(){
        if (Antnest.ignore_prefetch && typeof document.visibilityState !== "undefined" && document.visibilityState === "prerender") {
            Antnest.ignore_visitor = true;
        }
        if (Antnest.ignore_bots && searchBotRE.test(navigator.userAgent)) {
            Antnest.ignore_visitor = true;
        }
    }
    ...
    function notifyLoaders(){
        //notify load waiters
        var i;
        if (typeof Antnest.onload !== "undefined" && Antnest.onload.length > 0) {
            for (i = 0; i < Antnest.onload.length; i++) {
                if (typeof Antnest.onload[i] === "function") {
                    Antnest.onload[i]();
                }
            }
            Antnest.onload = [];
    ...
    function processWidget(currentWidget, hasSticker){
        try {
            ...
            iframe.src = Antnest.url + "/feedback?widget_id=" + currentWidget._id + "&app_key=" + Antnest.app_key + '&device_id=' + Antnest.device_id + '&sdk_version=' + SDK_VERSION;
            // inject them to dom
    ...
    function toRequestQueue(request) {
        ...
        if (!Antnest.app_key || !Antnest.device_id) {
            log("app_key or device_id is missing");
            return;
        }
        
        request.app_key = Antnest.app_key;
        request.device_id = Antnest.device_id;
        request.sdk_name = SDK_NAME;
        request.sdk_version = SDK_VERSION;
        
        if (Antnest.check_consent("location")) {
            if (Antnest.country_code) {
                request.country_code = Antnest.country_code;
            }

            if (Antnest.city) {
                request.city = Antnest.city;
            }

            if (Antnest.ip_address !== null) {
                request.ip_address = Antnest.ip_address;
            }
        }
        else {
            request.location = "";
    ...
    function heartBeat() {
        //ignore bots
        if (Antnest.test_mode || Antnest.ignore_visitor) {
            hasPulse = false;
            return;
        }
        ...
        var i = 0;
        //process queue
        if (typeof Antnest.q !== "undefined" && Antnest.q.length > 0) {
            var req;
            var q = Antnest.q;
            Antnest.q = [];
            for (i = 0; i < q.length; i++) {
                ...
                if (typeof req === "function") {
                    ...
                    if (typeof Antnest[req[0]] !== "undefined") {
                        Antnest[req[0]].apply(null, req.slice(1));
                    }
                    else {
                        var userdata = req[0].replace("userData.", "");
                        if (typeof Antnest.userData[userdata] !== "undefined") {
                            Antnest.userData[userdata].apply(null, req.slice(1));
                        }
                    }
                }
            }
        ...
        if (sessionStarted && autoExtend && trackTime) {
            ...
            if (last - lastBeat > sessionUpdate) {
                Antnest.session_duration(last - lastBeat);
                lastBeat = last;
            }
        }
        ...
        if (requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
            ...
            sendXmlHttpRequest(Antnest.url + apiPath, params, function(err, params){
    ...
    function getMetrics() {
        ...
        //getting app version
        metrics._app_version = Antnest.app_version;
    ...
    //log stuff
    function log() {
        if (Antnest.debug && typeof console !== "undefined") {
    ...
    function getConfig(key, ob, override) {
        if(typeof ob[key] !== "undefined")
            return ob[key];
        if(typeof Antnest[key] !== "undefined")
            return Antnest[key];
        return override;
    }

    function recordError(err, nonfatal, segments) {
        if (Antnest.check_consent("crashes") && err) {
    ...
    function sendXmlHttpRequest(url, params, callback) {
        try {
            ...
            var method = "GET";
            if (Antnest.force_post || data.length >= 2000) {
                method = "POST";
            }
            if(method === "GET") {
